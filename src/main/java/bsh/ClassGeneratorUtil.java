 ...
 * <p>
 *
 * <p>Stub classes contain all of the fields of a BeanShell scripted class as well as two "callback"
 * references to BeanShell namespaces: one for static methods and one for instance methods. Methods
 * of the class are delegators which invoke corresponding methods on either the static or instance
 * bsh object and then unpack and return the results. The static namespace utilizes a static import
 * to delegate variable access to the class' static fields. The instance namespace utilizes a
 * dynamic import (i.e. mixin) to delegate variable access to the class' instance variables.
 *
 * <p>
 *
 * <p>Constructors for the class delegate to the static initInstance() method of ClassGeneratorUtil
 * to initialize new instances of the object. initInstance() invokes the instance intializer code
 * (init vars and instance blocks) and then delegates to the corresponding scripted constructor
 * method in the instance namespace. Constructors contain special switch logic which allows the
 * BeanShell to control the calling of alternate constructors (this() or super() references) at
 * runtime.
 *
 * <p>
 *
 * <p>Specially named superclass delegator methods are also generated in order to allow BeanShell to
 * access overridden methods of the superclass (which reflection does not normally allow).
 *
 * <p>
 *
 * @author Pat Niemeyer
...
public class ClassGeneratorUtil implements Opcodes {
  ...
  public ClassGeneratorUtil(
    ...
    this.type = type;
    if (packageName != null) this.fqClassName = packageName.replace('.', '/') + "/" + className;
    else this.fqClassName = className;
  ...
  /**
   * Translate bsh.Modifiers into ASM modifier bitflags. Only a subset of modifiers are baked into
   * classes.
   */
  ...
  private static void generateField(
      String fieldName, String type, int modifiers, Object value, ClassWriter cw) {
  ...
  private static String getTypeParameterSignature(String[] paramTypes) {
    ...
    for (final String pt : paramTypes) sb.append(pt).append(":");
    return sb.toString();
  }
  ...
  private static String getMethodDescriptor(String returnType, String[] paramTypes) {
    StringBuilder sb = new StringBuilder("(");
    for (String paramType : paramTypes) sb.append(paramType);
  ...
  /**
   * Validate abstract method implementation. Check that class is abstract or implements all
   * abstract methods. BSH classes are not abstract which allows us to instantiate abstract classes.
  ...
  static void checkAbstractMethodImplementation(Class<?> type) {
    ...
    class Reflector {
      void gatherMethods(Class<?> type) {
        if (null != type.getSuperclass()) gatherMethods(type.getSuperclass());
        meths.addAll(Arrays.asList(type.getDeclaredMethods()));
        for (Class<?> i : type.getInterfaces()) gatherMethods(i);
      }
    }
    ...
    meths.stream()
        ...
        .forEach(
            method -> {
              ...
              if (meth.length > 0)
                checkInheritanceRules(
                    method.getModifiers(), meth[0].getModifiers(), method.getDeclaringClass());
            });
  }
      ...
      for (Method method : clas.getDeclaredMethods())
        if (method.getName().equals(methodName)
          ...
          for (int j = 0; j < paramTypes.length; j++)
            if (false == (found = paramTypes[j].equals(methodParamTypes[j]))) break;
  ...
  private static void generatePlainReturnCode(String returnType, MethodVisitor cv) {
    if (returnType.equals("V")) cv.visitInsn(RETURN);
      ...
      int opcode = IRETURN;
      if (returnType.equals("D")) opcode = DRETURN;
      else if (returnType.equals("F")) opcode = FRETURN;
  ...
  /**
   * Attempt to load a script named for the class: e.g. Foo.class Foo.bsh. The script is expected to
   * (at minimum) initialize the class body. That is, it should contain the scripted class
   * definition.
   *
   * <p>This method relies on the fact that the ClassGenerator generateClass() method will detect
   * that the generated class already exists and initialize it rather than recreating it.
   *
   * <p>The only interact that this method has with the process is to initially cache the correct
   * class in the class manager for the interpreter to insure that it is found and associated with
   * the scripted body.
   */
  public static void startInterpreterForClass(Class<?> genClass) {
    ...
    if (null == url)
      throw new InterpreterError(
          "Script (" + resName + ") for BeanShell generated class: " + genClass + " not found.");
  ...
  /**
   * This method provides a hook for the class generator implementation to store additional
   * information in the class's bsh static namespace. Currently this is used to store an array of
   * consructors corresponding to the constructor switch in the generated class.
   *
   * <p>This method must be called to initialize the static space even if we are using a previously
   * generated class.
   */
  public void initStaticNameSpace(NameSpace classStaticNameSpace, BSHBlock instanceInitBlock) {
    try {
      classStaticNameSpace.setLocalVariable(
          "" + BSHCLASSMODIFIERS, classModifiers, false /*strict*/);
  ...
  /**
   * Generate support code needed for Enum types. Generates enum values and valueOf methods, default
   * private constructor with initInstance call. Instead of maintaining a synthetic array of enum
  ...
  private void generateEnumSupport(
      String fqClassName, String className, String classDescript, ClassWriter cw) {
  ...
  private void generateEnumStaticInit(String fqClassName, String classDescript, MethodVisitor cv) {
    ...
    for (Variable var : vars)
      if (var.hasModifier("enum")) {
        ...
        if (ICONST_5 >= ordinal) cv.visitInsn(ordinal++);
        else cv.visitIntInsn(BIPUSH, ordinal++ - ICONST_0);
  ...
  /**
   * Generate a delegate method - static or instance. The generated code packs the method arguments
   * into an object array (wrapping primitive types in bsh.Primitive), invokes the static or
   * instance This invokeMethod() method, and then returns the result.
   */
  ...
  void generateConstructor(int index, String[] paramTypes, int modifiers, ClassWriter cw) {
    ...
    MethodVisitor cv =
        cw.visitMethod(modifiers, "<init>", methodDescriptor, paramTypesSig, exceptions);
  ...
  void generateStaticInitializer(ClassWriter cw) {
    ...
    cv.visitMethodInsn(
        INVOKESTATIC, "bsh/This", "pullBshStatic", "(Ljava/lang/String;)Lbsh/This;", false);
    cv.visitFieldInsn(PUTSTATIC, fqClassName, BSHSTATIC + className, "Lbsh/This;");

    if (type == ENUM) generateEnumStaticInit(fqClassName, classDescript, cv);
  ...
  /**
   * Generate a switch with a branch for each possible alternate constructor. This includes all
   * superclass constructors and all constructors of this class. The default branch of this switch
   * is the default superclass constructor.
   *
   * <p>This method also generates the code to call the static ClassGeneratorUtil
   * getConstructorArgs() method which inspects the scripted constructor to find the alternate
   * constructor signature (if any) and evaluate the arguments at runtime. The getConstructorArgs()
   * method returns the actual arguments as well as the index of the constructor to call.
   */
  void generateConstructorSwitch(int consIndex, int argsVar, int consArgsVar, MethodVisitor cv) {
    Label defaultLabel = new Label();
    Label endLabel = new Label();
    List<Invocable> superConstructors =
        BshClassManager.memberCache.get(superClass).members(superClass.getName());
    ...
    cv.visitMethodInsn(
        ...
        "bsh/This",
        "getConstructorArgs",
        "(Ljava/lang/Class;Lbsh/This;[Ljava/lang/Object;I)" + "Lbsh/This$ConstructorArgs;",
        false);
    ...
    for (int i = 0; i < superConstructors.size(); i++, index++)
      doSwitchBranch(
          index,
          superClassName,
          superConstructors.get(i).getParamTypeDescriptors(),
          endLabel,
          labels,
          consArgsVar,
          cv);
   ...
   * generateConstructorSwitch. The code generated by this method assumes that the argument array is
   * on the stack.
   *
  ...
  private void doSwitchBranch(
    ...
    for (String type : paramTypes) {
      ...
      String retType;
      if (method.equals("getObject")) retType = OBJECT;
      else retType = type;
  ...
  /**
   * Generate a superclass method delegate accessor method. These methods are specially named
   * methods which allow access to overridden methods of the superclass (which the Java reflection
   * API normally does not allow).
   */
  ...
  private void generateSuperDelegateMethod(
    ...
    for (String paramType : paramTypes) {
      if (isPrimitive(paramType)) cv.visitVarInsn(ILOAD, localVarIndex);
      else cv.visitVarInsn(ALOAD, localVarIndex);
  ...
  /**
   * Generates the code to reify the arguments of the given method. For a method "int m (int i,
   * String s)", this code is the bytecode corresponding to the "new Object[] { new
   * bsh.Primitive(i), s }" expression.
   *
  ...
  private void generateParameterReifierCode(
    ...
    for (int i = 0; i < paramTypes.length; ++i) {
      ...
      if (isPrimitive(param)) {
        int opcode;
        if (param.equals("F")) opcode = FLOAD;
        else if (param.equals("D")) opcode = DLOAD;
        else if (param.equals("J")) opcode = LLOAD;
        else opcode = ILOAD;
  ...
  /**
   * Generates the code to unreify the result of the given method. For a method "int m (int i,
   * String s)", this code is the bytecode corresponding to the "((Integer)...).intValue()"
   * expression.
   *
   * @author Eric Bruneton
   * @author Pat Niemeyer
